 ...
 * @file radio.cpp 
 * 
 * Comtains functions for driving the Si4432 RF chip
 * 
 */
#include "radio.h"
#include "timer.h"
...
uint8_t receive_packet_length;
uint8_t partial_packet_length;
uint8_t last_rssi;
uint8_t netid[2];

static volatile char packet_received;
static volatile char preamble_detected;
...
//eos: perhaps this must be changed to do the same in arduino

#define EX0_SAVE_DISABLE
#define EX0_RESTORE 
...
static const uint8_t reg_index[NUM_RADIO_REGISTERS] = {
...
static uint8_t air_data_rates[NUM_DATA_RATES] = {
	2,	4,	8,	16,	19,	24,	32,	48,	64,	96,	128,	192,	250
};

// register table for 433MHz radios
static uint8_t reg_table_433[NUM_RADIO_REGISTERS][NUM_DATA_RATES] = {
...
static uint8_t reg_table_470[NUM_RADIO_REGISTERS][NUM_DATA_RATES] = {
...
static uint8_t reg_table_868[NUM_RADIO_REGISTERS][NUM_DATA_RATES] = {
...
static  uint8_t reg_table_915[NUM_RADIO_REGISTERS][NUM_DATA_RATES] = {
	{0x01,	0x01,	0x01,	0x01,	0x01,	0x01,	0x05,	0x0B,	0x9A,	0x88,	0x8A,	0x8C,	0x8D},
	{0x03,	0x03,	0x03,	0x03,	0x03,	0x03,	0x03,	0x03,	0x03,	0x03,	0x03,	0x03,	0x03},
	{0xD0,	0xE8,	0xF4,	0xFA,	0xD3,	0xA7,	0x7D,	0x53,	0x5E,	0x7D,	0x5E,	0x3F,	0x30},
	{0xE0,	0x60,	0x20,	0x00,	0x00,	0x00,	0x01,	0x01,	0x01,	0x01,	0x01,	0x02,	0x02},
...
//
bool
radio_receive_packet(uint8_t *length, uint8_t * buf)
{
	uint8_t gout[3];
	uint16_t crc1, crc2;
	uint8_t errcount = 0;
...
//
static void
radio_write_transmit_fifo(register uint8_t n, uint8_t * buffer)
{

	rfSEL_SELECT;
    spi4teensy3::send(0x80 | EZRADIOPRO_FIFO_ACCESS);
    spi4teensy3::send(buffer,n);
	rfSEL_DESELECT;
...
//
static bool
radio_transmit_simple(uint8_t length, uint8_t * buf, uint16_t timeout_ticks)
{
	uint16_t tstart;
	bool transmit_started;
	uint8_t n;

	//debug("radio_transmit->length:%u timeout:%u\n",length,timeout_ticks);
...
//
static bool
radio_transmit_golay(uint8_t length, uint8_t * buf, uint16_t timeout_ticks)
{
	uint16_t crc;
	uint8_t gin[3];
	uint8_t elen, rlen;
...
//
bool
radio_transmit(uint8_t length, uint8_t * buf, uint16_t timeout_ticks)
{
	bool ret;
...
radio_receiver_on(void)
{
	...
	//EX0 = 1;
	//debug("Reg:%u",register_read(EZRADIOPRO_MODULATION_MODE_CONTROL_1));
...
//
bool
radio_configure(uint8_t air_rate)
{
	uint8_t i, rate_selection, control;
	...
	static const uint8_t power_levels[NUM_POWER_LEVELS] = { 1, 2, 5, 8, 11, 14, 17, 20 };
...
//	EX0_SAVE_DISABLE;
	...
	rfSEL_SELECT;
	spi4teensy3::send(reg | 0x80);
	spi4teensy3::send(value);
	rfSEL_DESELECT;	                          // drive NSS high
...
static void
read_receive_fifo(register uint8_t n, uint8_t * buf) 
{

	rfSEL_SELECT;
	spi4teensy3::send(EZRADIOPRO_FIFO_ACCESS);
	spi4teensy3::receive(buf,n);
	rfSEL_DESELECT;
}
...
void rfInterrupt(void)
{
	...
	if (status & EZRADIOPRO_IPKVALID) {
		uint8_t len = register_read(EZRADIOPRO_RECEIVED_PACKET_LENGTH);
